...
import { BiPlus, BiUserVoice, BiBook, BiBrain, BiListUl } from 'react-icons/bi';
import {
  Box,
  Flex,
  Icon,
...
import { getItem, IUser } from '@clevery-lms/data';
...
export default function RankingTable() {
  ...
  const [columnSettings, setColumnSettings] = useState<ColumnSettings[]>([
    {
      ...
      value: 'nombre',
      checked:
        typeof getItem('user_alumnos_nombre') === 'boolean'
          ? getItem('user_alumnos_nombre')
          : true,
      onClick: () => {
        checkColumn('alumnos', 'nombre', columnSettings, setColumnSettings);
      },
    },
    {
      label: 'Hoja de ruta',
      value: 'ruta',
      checked:
        typeof getItem('user_alumnos_ruta') === 'boolean'
          ? getItem('user_alumnos_ruta')
          : true,
      onClick: () => {
        checkColumn('alumnos', 'ruta', columnSettings, setColumnSettings);
      },
    },
  ]);
  ...
  const getUsuariosBy = async (params: any, query: string = '') => {
    ...
    return await getUsersStats({ query: [{ ...auxQuery, page }] });
  };
  ...
  const columns: OpenColumn[] = [
    ...
    },
    {
      ...
      filterable: true,
      options: [
        { label: 'Incubación', value: 'incubacion' },
